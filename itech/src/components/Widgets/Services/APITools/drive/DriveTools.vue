            ...
            <template v-slot:footer>
                <div class="flex items-center justify-center my-2 text-sm pt-3">
                    ...
                    <button class="px-3 py-2 rounded mx-2 bg-red-500 hover:bg-red-300 text-white"
                        @click="deleteDriveProjectFolder">Delete</button>
                </div>
            </template>
        </ModalBox>
    </Transition>
...
const alertDelete = function(id){
    driveFolderInput.value.selectedId = id
    driveFolderInput.value.value = ''
    deleteAlertBox.value.show = true
}
const deleteDriveProjectFolder = ()=>{
    store.dispatch(`deleteDriveProjectFolder`,{pid: props.id, driveId: driveFolderInput.value.selectedId}).then((res)=>{
        if(res.ok){
