<template>
    <div style="z-index: 100" class="fixed w-full h-full bg-slate-500/50 backdrop-blur-sm modal" v-if="show" @click="hadlerClose">
        ...
        <div @click.stop="" :class="[`${width ? width:'w-[300px]'}`,`${height ? height:'h-[fit-content]'}`]" class="absolute border-gray-300 border-2 rounded-lg bg-gray-200 modal-box overflow-hidden flex flex-col justify-start" v-if="show"
        >
